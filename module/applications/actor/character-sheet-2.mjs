...
export default class ActorSheet5eCharacter2 extends ActorSheet5eCharacter {
  constructor(object, options = {}) {
    ...
    const { width, height } =
      game.user.getFlag("dnd5e", `sheetPrefs.${key}`) ?? {};
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      ...
      dragDrop: [
        ...
        {
          dragSelector: ".favorites :is([data-item-id], [data-effect-id])",
          dropSelector: null,
        },
        {
          dragSelector: ".classes .gold-icon[data-item-id]",
          dropSelector: null,
        },
        {
          dragSelector: "[data-key] .skill-name, [data-key] .tool-name",
          dropSelector: null,
        },
        {
          dragSelector: ".spells-list .spell-header, .slots[data-favorite-id]",
          dropSelector: null,
        },
      ],
  ...
  get template() {
    if (!game.user.isGM && this.actor.limited)
      return "systems/dnd5e/templates/actors/limited-sheet-2.hbs";
  ...
  async _renderOuter() {
    ...
    nav.append(
      ...this.constructor.TABS.map(({ tab, label, icon, svg }) => {
  ...
  async getData(options) {
    ...
    const sidebarCollapsed = game.user.getFlag(
      "dnd5e",
    ...
    // Class
    context.labels.class = Object.values(this.actor.classes)
      .sort((a, b) => {
    ...
    const showTokenPortrait =
      this.actor.getFlag("dnd5e", "showTokenPortrait") === true;
    ...
    context.exhaustion = Array.fromRange(max, 1).reduce(
      (acc, n) => {
    ...
    // Speed
    context.speed = Object.entries(CONFIG.DND5E.movementTypes).reduce(
      (obj, [k, label]) => {
    ...
    context.hd.pct = Math.clamped(
      context.hd.max ? (context.hd.value / context.hd.max) * 100 : 0,
      0,
      100
    );
    ...
    // Ability Scores
    context.abilityRows = Object.entries(context.abilities).reduce(
      (obj, [k, ability]) => {
        ability.key = k;
    ...
    context.abilityRows.optional =
      Object.keys(CONFIG.DND5E.abilities).length - 6;
    ...
    for (let ability of Object.values(context.abilities)) {
      ...
      ability.class =
        this.constructor.PROFICIENCY_CLASSES[
          context.editable ? ability.baseProf : ability.proficient
        ];
    ...
    // Skills & Tools
    for (const [key, entry] of Object.entries(context.skills).concat(
      Object.entries(context.tools)
    )) {
      entry.class =
        this.constructor.PROFICIENCY_CLASSES[
          context.editable ? entry.baseValue : entry.value
        ];
      entry.sign = Math.sign(entry.total) < 0 ? "-" : "+";
      entry.mod = Math.abs(entry.total);
      if (key in CONFIG.DND5E.skills)
        entry.reference = CONFIG.DND5E.skills[key].reference;
      else if (key in CONFIG.DND5E.toolIds)
        entry.reference = Trait.getBaseItemUUID(CONFIG.DND5E.toolIds[key]);
    }
    ...
    if (details.race instanceof dnd5e.documents.Item5e)
      context.race = details.race;
    if (details.background instanceof dnd5e.documents.Item5e)
      context.background = details.background;

    // Senses
    context.senses = Object.entries(CONFIG.DND5E.senses).reduce(
      (obj, [k, label]) => {
    ...
    if (attributes.senses.special)
      attributes.senses.special.split(";").forEach((v, i) => {
    ...
    const msak = simplifyBonus(
      this.actor.system.bonuses.msak.attack,
      context.rollData
    );
    const rsak = simplifyBonus(
      this.actor.system.bonuses.rsak.attack,
      context.rollData
    );

    for (const item of Object.values(this.actor.classes).sort(
      ...
      context.spellcasting.push({
        ...
        ability: {
          sign: Math.sign(mod) < 0 ? "-" : "+",
          value: Math.abs(mod),
          ability: sc.ability,
        },
        attack: {
          sign: Math.sign(attack) < 0 ? "-" : "+",
          value: Math.abs(attack),
        },
    ...
    context.conditions = Object.entries(CONFIG.DND5E.conditionTypes).reduce(
      (arr, [k, c]) => {
        if (k === "diseased") return arr; // Filter out diseased as it's not a real condition.
        //if (k === "exhaustion") return arr; // Filter out exhaustion as it's not a real condition.
        // Filter out diseased custom effects
        if (k.startsWith("CE")) return arr;
    ...
    for (const [trait, config] of Object.entries(CONFIG.DND5E.traits)) {
      ...
      values = values.map((key) => {
        ...
        if (data.bypasses?.size && CONFIG.DND5E.damageTypes[key]?.isPhysical)
          icons.push(...data.bypasses);
        return value;
      });
      if (data.custom)
        data.custom.split(";").forEach((v) => values.push({ label: v.trim() }));
  ...
  _prepareItems(context) {
    ...
    context.features = context.features.filter(
      (f) => f.dataset.type !== "background" && f.dataset.type !== "race"
    );
    ...
    if (
      this.actor.system.details.background instanceof dnd5e.documents.Item5e
    ) {
      context.features.push({
        label: "DND5E.FeaturesBackground",
        items: [],
    ...
    if (item.type === "spell") {
      ...
      if (mode === "always" || mode === "prepared") {
        ...
        ctx.preparation = {
          ...
          cls: prepared ? "active" : "",
          icon: `<i class="fa-${prepared ? "solid" : "regular"} fa-${
            isAlways ? "certificate" : "sun"
          }"></i>`,
          title: isAlways
  ...
  activateListeners(html) {
    ...
    html
      .find("[data-item-id][data-action]")
      .on("click", this._onItemAction.bind(this));
    html
      .find(".rollable:is(.saving-throw, .ability-check)")
      .on("click", this._onRollAbility.bind(this));
    html.find("proficiency-cycle").on("change", this._onChangeInput.bind(this));
    html
      .find(".sidebar .collapser")
      .on("click", this._onToggleSidebar.bind(this));
    this.form
      .querySelectorAll(".item-tooltip")
      .forEach(this._applyItemTooltips.bind(this));
    this.form
      .querySelectorAll("[data-reference-tooltip]")
      .forEach(this._applyReferenceTooltips.bind(this));
    ...
    const featuresElement = html[0].querySelector(
      `[data-tab="features"] ${this.options.elements.inventory}`
    );
    if (featuresElement)
      new ContextMenu5e(html, ".pills-lg [data-item-id]", [], {
        onOpen: (...args) => featuresElement._onOpenContextMenu(...args),
      });

    if (this.isEditable) {
      html
        .find(".meter > .hit-points")
        .on("click", (event) => this._toggleEditHP(event, true));
      html
        .find(".meter > .hit-points > input")
        .on("blur", (event) => this._toggleEditHP(event, false));
  ...
  _disableFields(form) {
    ...
    form
      .querySelectorAll(".interface-only")
  ...
  _onChangeTab(event, tabs, active) {
    ...
    const sidebarCollapsed = game.user.getFlag(
      "dnd5e",
      `sheetPrefs.character.tabs.${active}.collapseSidebar`
    );
    if (sidebarCollapsed !== undefined) this._toggleSidebar(sidebarCollapsed);
    const createChild = this.form.querySelector(".create-child");
    createChild.setAttribute(
      "aria-label",
      game.i18n.format("SIDEBAR.Create", {
        type: game.i18n.localize(
          `DOCUMENT.${active === "effects" ? "ActiveEffect" : "Item"}`
        ),
      })
    );
  }
  ...
  _onDragStart(event) {
    ...
    const isSlots =
      ...
      event.target.classList.contains("spell-header");
    let type;
    ...
    if (type === "slots")
      dragData.dnd5e.id = preparationMode === "pact" ? "pact" : `spell${level}`;
  ...
  _toggleDeathTray(open) {
    ...
    tab.dataset.tooltip = `DND5E.DeathSave${
      this._deathTrayOpen ? "Hide" : "Show"
    }`;
  ...
  _toggleSidebar(collapsed) {
    ...
    collapser.dataset.tooltip = `JOURNAL.View${
      collapsed ? "Expand" : "Collapse"
    }`;
    collapser.setAttribute(
      "aria-label",
      game.i18n.localize(collapser.dataset.tooltip)
    );
  ...
  _onAction(event) {
    ...
    switch (target.dataset.action) {
      case "findItem":
        this._onFindItem(target.dataset.itemType);
        break;
      case "removeFavorite":
        this._onRemoveFavorite(event);
        break;
      case "spellcasting":
        this._onToggleSpellcasting(event);
        break;
      case "toggleInspiration":
        this._onToggleInspiration();
        break;
      case "useFavorite":
        this._onUseFavorite(event);
        break;
    }
  }
  ...
  _onChangeInput(event) {
    ...
    if (event.target.closest(".favorites") && name && item)
      return item.update({ [name]: event.target.value });
      ...
      return Item.implementation.create(
        {
          name: game.i18n.format("DOCUMENT.New", {
            type: game.i18n.format(CONFIG.Item.typeLabels.spell),
          }),
          type: "spell",
          img:
            Item.implementation.getDefaultArtwork({ type: "spell" })?.img ??
            Item.implementation.DEFAULT_ICON,
        },
        { parent: this.actor, renderSheet: true }
      );
  ...
  _onFindItem(type) {
    switch (type) {
      case "class":
        game.packs.get("dnd5e.classes").render(true);
        break;
      case "race":
        game.packs.get("dnd5e.races").render(true);
        break;
      case "background":
        game.packs.get("dnd5e.backgrounds").render(true);
        break;
    }
  }
  ...
  _onToggleInspiration() {
    this.actor.update({
      "system.attributes.inspiration":
        !this.actor.system.attributes.inspiration,
    });
  }
  ...
  _onRollAbility(event) {
    const abilityId =
      event.currentTarget.closest("[data-ability]").dataset.ability;
    const isSavingThrow =
      event.currentTarget.classList.contains("saving-throw");
  ...
  _onRemoveFavorite(event) {
    const { favoriteId } =
      event.currentTarget.closest("[data-favorite-id]")?.dataset ?? {};
  ...
  _onUseFavorite(event) {
    ...
    if (favorite instanceof dnd5e.documents.Item5e)
      return favorite.use({}, { event });
    if (favorite instanceof dnd5e.documents.ActiveEffect5e)
  ...
  async _prepareFavorites() {
    // Legacy resources
    const resources = Object.entries(this.actor.system.resources).reduce(
      (arr, [k, r]) => {
    ...
    return resources.concat(
      await this.actor.system.favorites.reduce(async (arr, f) => {
        ...
        if (modifier !== undefined) {
          ...
          if (!isNaN(value))
            modifier = { abs: Math.abs(value), sign: value < 0 ? "-" : "+" };
        }
        ...
        arr.push({
          ...
          parentId:
            type === "effect" && favorite.parent !== favorite.target
              ...
              : null,
          preparationMode:
            type === "slots" ? (id === "pact" ? "pact" : "prepared") : null,
  ...
  async _getFavoriteData(type, id) {
      ...
      if (id === "pact")
        return {
          ...
          subtitle: [
            ...
            game.i18n.localize("DND5E.AbbreviationSR"),
          ],
    ...
    // Skills & Tools
      ...
      if (type === "tool") {
        ...
        ({ img, name: title } = Trait.getBaseItem(reference, {
          indexOnly: true,
        }));
      } else if (type === "skill")
